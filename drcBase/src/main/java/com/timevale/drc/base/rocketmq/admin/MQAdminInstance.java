...
public class MQAdminInstance {
    private static final ThreadLocal<DefaultMQAdminExt> MQ_ADMIN_EXT_THREAD_LOCAL = new ThreadLocal<DefaultMQAdminExt>() {
        @Override
        protected DefaultMQAdminExt initialValue() {
            try {
                initMQAdminInstance(60 * 1000);
            } catch (MQClientException e) {
                e.printStackTrace();
            }
            return MQ_ADMIN_EXT_THREAD_LOCAL.get();
        }
    };
    ...
    public static void initMQAdminInstance(long timeoutMillis) throws MQClientException {
        ...
        if (nowCount == null) {
            ...
            if (timeoutMillis > 0) {
                defaultMQAdminExt = new DefaultMQAdminExt(AclUtil.getAclRPCHook(), timeoutMillis);
            } else {
                defaultMQAdminExt = new DefaultMQAdminExt(AclUtil.getAclRPCHook());
            }
